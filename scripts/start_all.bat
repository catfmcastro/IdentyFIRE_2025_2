...
start "IdentyFire - Servidor" cmd /k "cd /d %CD% && .venv\Scripts\activate.bat && python src\server_gui.py"

timeout /t 3 /nobreak > nul

echo Iniciando Cliente em nova janela...
start "IdentyFire - Cliente" cmd /k "cd /d %CD% && .venv\Scripts\activate.bat && python src\client_gui.py"

timeout /t 2 /nobreak > nul

echo Iniciando Treinamento em nova janela...
start "IdentyFire - Treinamento" cmd /k "cd /d %CD% && .venv\Scripts\activate.bat && python src\training_gui.py"
